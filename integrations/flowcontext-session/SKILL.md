...
description: 在每个 Codex 任务开始时把当前工作区路由到唯一 Project、Topic Card 与 Session；继续已有主题时在同一 Topic 下登记新 Session。
---
...
2. 读取该 Project 的开放 Topic Cards。若用户表达“继续此主题”、接续或基于最近 Handoff 继续，且仅有一个高置信度匹配的开放 Topic Card，则复用该开放 Topic Card；不得仅以工作区路径新建 Project 或 Topic Card。首条请求若高置信度属于一个新主题，才可以创建新的 Topic Card，并在对用户的正常回复中说明已创建；高置信度的判断必须有请求标题、下一步或现有上下文依据。
...
5. 调用 `flowcontext session start --json <临时 JSON 文件>` 登记 Session，输入必须包含 `topicCardId`、Codex 技术 thread ID、设备 ID、设备平台（`macos` 或 `windows`）、工作区路径和开始时间。新 Codex 任务必须以新的 Codex thread ID 创建新的 Session，不要把 JSON 正文放在命令参数中。
