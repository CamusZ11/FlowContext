...
5. 调用 `flowcontext session start --json <临时 JSON 文件>` 登记 Session，输入必须包含 `topicCardId`、Codex 技术 thread ID、设备 ID、当前平台（`macos` 或 `windows`）、工作区路径和开始时间。不要把 JSON 正文放在命令参数中。
