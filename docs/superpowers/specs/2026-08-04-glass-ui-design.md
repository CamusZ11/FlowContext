...
## 信息结构
Header 使用紧凑的单行品牌结构：左侧为 FlowContext 图标与品牌名称，右侧为同步状态。Desktop 不显示页面级主标题，不提供关闭或退出按钮。To-do 与建议继续是首要内容区块，Daily Lens、Codex Reports 和 Project Groups 保留原有 disclosure 语义。
## 交互与可访问性
